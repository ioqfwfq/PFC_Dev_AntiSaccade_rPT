function [ltc, p] = Neuron_Data_latency_AntiSaccade_4rPT_alignsac
...
for n = 1:length(Neurons)
    ...
    try
        [allTS1_temp, allTS2_temp, allTS3_temp, allTS4_temp, RT1_temp, RT2_temp, RT3_temp, RT4_temp] = Get_allTS_AllTrials_4rawProcessingTime_alignSac(Antifilename,best_target(n));
...
[on1_temp, off1_temp] = Neuron_Data_QsumLatency_alignSac(raster1_temp,stimon1_temp,maxoff1_temp,allTS1,bin_edges1_temp);
[raster2_temp, stimon2_temp, maxoff2_temp, bin_edges2_temp] = Get_raster_for_latency(allTS2);
[on2_temp, off2_temp] = Neuron_Data_QsumLatency_alignSac(raster2_temp,stimon2_temp,maxoff2_temp,allTS2,bin_edges2_temp);
[raster3_temp, stimon3_temp, maxoff3_temp, bin_edges3_temp] = Get_raster_for_latency(allTS3);
[on3_temp, off3_temp] = Neuron_Data_QsumLatency_alignSac(raster3_temp,stimon3_temp,maxoff3_temp,allTS3,bin_edges3_temp);
[raster4_temp, stimon4_temp, maxoff4_temp, bin_edges4_temp] = Get_raster_for_latency(allTS4);
[on4_temp, off4_temp] = Neuron_Data_QsumLatency_alignSac(raster4_temp,stimon4_temp,maxoff4_temp,allTS4,bin_edges4_temp);
ltc(1) = on1_temp;
ltc(2) = on2_temp;
ltc(3) = on3_temp;
ltc(4) = on4_temp;
p = Neuron_Data_latency_AntiSaccade_4rPT_alignsac_permutationtest(Neurons,RT1,RT2,RT3,RT4,ltc)
...
bin_width = 0.1;  % 10 milliseconds bin
...
stimon_temp = find(bin_edges == -0.35);
