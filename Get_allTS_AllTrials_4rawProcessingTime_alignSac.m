    ...
    for m1 = 1:length(MatData.class(class_num).ntr)
        if ~isempty(MatData.class(class_num).ntr(m1).Saccade_onT)
            try
                ...
                TS_sacon = MatData.class(class_num).ntr(m1).TS-MatData.class(class_num).ntr(m1).Saccade_onT;
            catch
            end
            if RT < Threshold1
                allTS1 = [allTS1 TS_sacon];
                RT1 = [RT1 RT];
            end
            if RT >= Threshold1 && RT < Threshold2
                allTS2 = [allTS2 TS_sacon];
                RT2 = [RT2 RT];
            end
            if  RT >= Threshold2 && RT < Threshold3
                allTS3 = [allTS3 TS_sacon];
                RT3 = [RT3 RT];
            end
            if RT >= Threshold3
                allTS4 = [allTS4 TS_sacon];
                RT4 = [RT4 RT];
            end
        end
    end
        ...
        for m2 = 1:length(MatData.class(class_num + 8).ntr)
            if ~isempty(MatData.class(class_num + 8).ntr(m2).Saccade_onT)
                try
                    ...
                    TS_sacon = MatData.class(class_num + 8).ntr(m2).TS-MatData.class(class_num + 8).ntr(m2).Saccade_onT;
                catch
                end
                if RT < Threshold1
                    allTS1 = [allTS1 TS_sacon];
            ...
            for m3 = 1:length(MatData.class(class_num + 16).ntr)
                if ~isempty(MatData.class(class_num + 16).ntr(m3).Saccade_onT)
                    try
                        ...
                        TS_sacon = MatData.class(class_num + 16).ntr(m3).TS-MatData.class(class_num + 16).ntr(m3).Saccade_onT;
                    catch
                    end
                    if RT < Threshold1
                        allTS1 = [allTS1 TS_sacon];
