function Neuron_Data_PSTH_AntiSaccade_fasttrials_alignsac
% For AntiSaccade task
% Plot result from fast (rPT <= 120 ms) trials
...
%     Errfilename = [Neurons1{n,1}(1:6),'_2_',num2str(Neurons1{n,2}),'_erriscuesac'];
    try
        [psth_temp, ntrs_temp] = Get_PsthM_fastTrials_alignSac(Antifilename,best_target1(n));
        psth1(n,:) = psth_temp;
        ntrs1(n) = ntrs_temp;
    catch
...
%     Errfilename = [Neurons2{n,1}(1:6),'_2_',num2str(Neurons2{n,2}),'_erriscuesac'];
    try
        [psth_temp, ntrs_temp] = Get_PsthM_fastTrials_alignSac(Antifilename,best_target2(n));
        psth2(n,:) = psth_temp;
        ntrs2(n) = ntrs_temp;
    catch
